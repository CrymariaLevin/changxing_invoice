...
def data_merge(com, original_data, path):
    ...
    if len(oil_list) < 1 or len(com_list) < 1: # 对应解析失败
    ...
    elif oil_num == 1 and com_num > 1:  # 一个油品多个公司：
        if flag == 0:
            # xf = com_list[0]
            # gf = com
            # xf = name_filter(com_list[0])
        ...
        Dj = original_data[0][26] if original_data[0][26] else None
        if ',' in str(Dj):
            Dj = round(float(Dj.replace(',', '')),2)
        else:
            Dj = round(float(Dj),2)
        Sl_t = original_data[0][25] if original_data[0][25] else 0
        if ',' in str(Sl_t):
            Sl_t = round(float(Sl_t.replace(',', '')),2)
        else:
            Sl_t = round(float(Sl_t),2)
        for n in original_data:
            if '税' in n[8] and Sl_t > 0:
                tax_all = float(original_data[0][16] if original_data[0][16] else 0) + float(n[16] if n[16] else 0)
                Dj_t = round(tax_all / Sl_t, 2)
            else:
                continue
        if not tax_all:
            print('无法解析：单个产品对应多家公司')
            print('oil_num:', oil_num, oil_list)
            print('com_num:', com_num, com_list)
            return -2
        for n in original_data:
            if any(c in n[8] for c in com_list):
                xf_list = [com for com in n[8].replace(' -- ', ' - ').replace(' ', '').split('-') if
                           any(key in com for key in com_key)]
                if len(xf_list) == 0:
                    print('公司名称解析错误')
                    return -2
                xf = xf_list[0]
